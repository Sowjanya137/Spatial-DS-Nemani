...
class DrawGeoJson(object):
    ...
    def __init__(self,screen,width,height):
        ...
        self.colors = Colors('C://Users//sowja//Documents//SPDS//Json_Files//colors.json')
...
class DrawingFacade(object):
    def __init__(self,width,height):
        ...
        below.
        """
        self.sb = StateBorders('C://Users//sowja//Documents//SPDS//Json_Files//state_borders.json')
        self.wc = WorldCountries('C://Users//sowja//Documents//SPDS//Json_Files//countries.geo.json')
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                 ...
                 pos=(mx,my)
                 #print("Mouse Clicked at Postion: ",pos)
                 for key,value in gd.adjusted_poly_dict.items():
                    #print(value)
                    for lst in value:
                        is_there=point_inside_polygon(mx,my,lst)
                        max_x,max_y=map(max,zip(*lst))
                        min_x,min_y=map(min,zip(*lst))
                        
                        if(is_there):
                            print("Mouse Clicked in: ",key,is_there)
                            myfont = pygame.font.SysFont("monospace", 15)
                            #render text
                            label = myfont.render(str(key), 1, (0,0,0))
                            screen.blit(label, (mx, my))
                            #higlighting the border
                            pygame.draw.polygon(screen,color,lst,8)
                            #draws the rectangle on borders
                            print("Max_x: ",max_x)
                            print("Max_y: ",max_y)  
                            print("Min_x: ",min_x)
                            print("Min_y: ",min_y) 
                            width=max_x-min_x
                            height=max_y-min_y                          
                            pygame.draw.rect(screen, color, (min_x,min_y,width,height+10),1)
